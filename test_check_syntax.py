#!/usr/bin/env python3
"""
Small test client for the plantuml-mcp `check_syntax` tool over the
Streamable HTTP endpoint.

It performs the full MCP handshake and then calls `check_syntax` on a sample
...
Usage:
    ...
    python test_check_syntax.py
    python test_check_syntax.py --url http://localhost:8080/mcp
...
# The diagram whose syntax we want to check.
SAMPLE_DIAGRAM = "\n".join([
	"@startuml",
...
def main() -> int:
	parser = argparse.ArgumentParser(description="Test the plantuml-mcp check_syntax tool.")
	parser.add_argument(
		"--url",
	...
	# 3. tools/call: invoke check_syntax with the sample diagram.
	print("-> tools/call  check_syntax")
	print("   source:")
	...
	# The tool returns a CheckSyntaxRecord serialized as JSON, which Spring AI
	# wraps as a text content item. Try to pretty-print it as JSON; fall back to
	# the raw text if the shape differs.
	try:
		parsed = json.loads(raw)
		print(json.dumps(parsed, indent=2))
		if isinstance(parsed, dict) and parsed.get("valid") is True:
			print(f"\n=> valid {parsed.get('diagramType')}")
		elif isinstance(parsed, dict):
			print(f"\n=> invalid: line {parsed.get('errorLine')}: {parsed.get('errorMessage')}")
