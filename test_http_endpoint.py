#!/usr/bin/env python3
"""
Small test client for the plantuml-mcp Streamable HTTP endpoint.

It performs the full MCP handshake and then calls the `plantuml_version`
tool, printing the PlantUML version returned by the server.
...
Usage:
    ...
    python test_http_endpoint.py --url http://localhost:8080/mcp
...
def extract_version(tool_result: dict) -> str:
...
def main() -> int:
	parser = argparse.ArgumentParser(description="Test the plantuml-mcp HTTP endpoint.")
	parser.add_argument(
		"--url",
	...
	# 3. tools/call: invoke plantuml_version.
	print("-> tools/call  plantuml_version")
	call_payload = {
		"jsonrpc": "2.0",
		"id": 2,
		"method": "tools/call",
		"params": {"name": "plantuml_version", "arguments": {}},
	}
	call_result, session_id = post(args.url, call_payload, session_id)

	version = extract_version(call_result)
	print("\nPlantUML version reported by the server:")
	print(version)
	return 0
